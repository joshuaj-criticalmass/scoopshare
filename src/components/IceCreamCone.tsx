...
export function IceCreamCone({
  ...
  const animationClass =
    animationMode === "loop"
      ? " ice-cream-animation-loop"
      : animationMode === "once"
      ? " ice-cream-animation-once"
      : "";

  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="-40 -100 443.29 1214.28"
    ...
    >
      <defs>
        ...
        <filter id={`cone-shadow-${uid}`} x="-20%" y="-10%" width="140%" height="150%">
          <feDropShadow dx="0" dy="24" stdDeviation="18" floodColor="#8a4b16" floodOpacity="0.28" />
        </filter>
        <filter id={`scoop-shadow-${uid}`} x="-25%" y="-20%" width="150%" height="160%">
          <feDropShadow dx="0" dy="18" stdDeviation="14" floodColor="#8b4f2d" floodOpacity="0.22" />
        </filter>
      </defs>

      <g className={animationMode !== "off" ? `ice-cream-cone-rise${animationClass}` : undefined}>
        {/* ── Waffle cone (shadow layer) ── */}
        <path
          fill="#ff9933"
          filter={`url(#cone-shadow-${uid})`}
      ...
      <g
        className={
          animationMode !== "off"
            ? `ice-cream-scoop-drop ice-cream-scoop-drop-bottom${animationClass}`
            : undefined
        }
      >
        <path
      ...
      <g
        className={
          animationMode !== "off"
            ? `ice-cream-scoop-drop ice-cream-scoop-drop-middle${animationClass}`
            : undefined
        }
      >
        <path
      ...
      <g
        className={
          animationMode !== "off"
            ? `ice-cream-scoop-drop ice-cream-scoop-drop-top${animationClass}`
            : undefined
        }
      >
        <path
